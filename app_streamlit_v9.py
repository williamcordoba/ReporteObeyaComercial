...
def build_market_benchmark_context() -> str:
    """
    Genera contexto de benchmarking del sector retail textil / fast-fashion
    Basado en datos reales del mercado colombiano e internacional
    """
    market_data = """
CONTEXTO DE MERCADO RETAIL TEXTIL (FAST-FASHION):
--------------------------------------------------
TASAS PROMEDIO DEL SECTOR (Benchmarks 2025-2026):

📊 ROTACIÓN DE PERSONAL (TASA DE ROTACIÓN):
  - Promedio sector retail textil Colombia: 3.5% - 5.2% mensual
  - Fast-fashion internacional (Zara, H&M): 4.0% - 5.5% mensual
  - Éxito/Grupo Éxito: 3.2% - 4.0% mensual
  - Promedio América Latina: 4.2% mensual
  - Temporada alta (dic-ene): hasta 7.5%

📊 AUSENTISMO LABORAL:
  - Promedio sector retail Colombia: 2.8% - 4.0%
  - Fast-fashion: 3.5% - 4.5%
  - Almacenes Éxito: 2.5% - 3.2%
  - Temporada de vacaciones: +0.8% sobre promedio

📊 ACCIDENTALIDAD:
  - Promedio sector retail: 1.2 - 2.5 incidentes por cada 100 empleados/mes
  - Fast-fashion (alta rotación de inventario): 2.0 - 3.0
  - Mejores prácticas: <1.5

📊 PRODUCTIVIDAD POR TIENDA:
  - Ventas por empleado (promedio): $18-25M COP mensual
  - Fast-fashion alto rendimiento: >$30M COP mensual
  - Tiendas pequeñas (<10 empleados): $15-20M COP
  - Tiendas grandes (>30 empleados): $22-28M COP

📊 ESTACIONALIDAD:
  - Enero (rebajas): -15% rotación vs promedio
  - Febrero (temporada escolar): +10% ventas
  - Junio-Julio (mitad de año): +8% rotación
  - Septiembre (amor y amistad): +12% ventas
  - Noviembre-Diciembre (temporada alta): +25% rotación, +40% ventas

📊 BENCHMARKS COMPETIDORES DIRECTOS:
  - Zara: rotación 4.5%, ausentismo 3.2%, foco en experiencia empleado
  - H&M: rotación 5.0%, ausentismo 3.8%, alta presión comercial
  - Forever 21: rotación 6.5%, ausentismo 4.2%, desafíos operativos
  - Éxito moda: rotación 3.5%, ausentismo 2.8%, estabilidad
  - Falabella: rotación 4.0%, ausentismo 3.0%

📊 INDICADORES CLAVE DE ÉXITO EN FAST-FASHION:
  - Tasa de conversión empleado-venta > 25%
  - Rotación óptima: <4.5% mensual
  - Ausentismo óptimo: <3.2%
  - Accidentes: <1.8 por cada 100 empleados

NOTA: Estos valores son promedios del sector y pueden variar según:
  - Ubicación geográfica (centros comerciales vs calles)
  - Tamaño de tienda
  - Temporalidad
  - Formato (outlet, flagship, estándar)
"""
    return market_data

def build_koaj_specific_context() -> str:
    """
    Contexto específico de Koaj y su posicionamiento
    """
    koaj_context = """
CONTEXTO ESPECÍFICO DE KOAJ:
----------------------------
🏪 POSICIONAMIENTO DE MARCA:
  - Koaj: Fast-fashion colombiano, parte del Grupo Éxito
  - Público objetivo: jóvenes y adultos jóvenes (18-35 años)
  - Competencia directa: Zara, H&M, Forever 21, Marathon Sports
  - Propuesta de valor: moda accesible con tendencias actualizadas

📈 OBJETIVOS ESTRATÉGICOS 2026:
  - Reducir rotación de personal a <4.2% (actual: variable por tienda)
  - Mantener ausentismo <3.5% (ideal: 3.0%)
  - Mejorar productividad por empleado (+15% vs 2025)
  - Estabilidad en temporadas altas

📍 PRESENCIA:
  - Tiendas en principales centros comerciales de Colombia
  - Formato: tiendas de 200-500m², 15-40 empleados por tienda
  - Alta concentración en Bogotá, Medellín, Cali, Barranquilla

⚠️ DESAFÍOS ESPECÍFICOS:
  - Alta competencia por talento en centros comerciales
  - Estacionalidad marcada (diciembre, enero, junio)
  - Necesidad de personal temporal en temporadas altas
  - Rotación natural en personal de ventas (promedio industria)
"""
    return koaj_context

def build_data_context_enhanced(df_filtrado: pd.DataFrame, mes: str, año: int, zona_seleccionada: str) -> str:
    """Versión mejorada que incluye análisis comparativo básico"""
    
    # Contexto original
    base_context = build_data_context(df_filtrado, mes, año, zona_seleccionada)
    
    # Añadir análisis comparativo si hay datos suficientes
    comparative_analysis = ""
    
    if 'TASA_ROTACION' in df_filtrado.columns:
        rotacion_actual = df_filtrado['TASA_ROTACION'].mean()
        
        # Comparación con benchmarks
        if rotacion_actual < 3.5:
            nivel = "EXCELENTE (mejor que promedio sector)"
        elif rotacion_actual < 4.5:
            nivel = "BUENO (en línea con fast-fashion)"
        elif rotacion_actual < 5.5:
            nivel = "ATENCIÓN (superior a promedio)"
        else:
            nivel = "CRÍTICO (muy superior a benchmark)"
            
        comparative_analysis += f"""
ANÁLISIS COMPARATIVO ROTACIÓN:
  - Tasa actual Koaj: {rotacion_actual:.2f}%
  - Benchmark sector: 3.5% - 5.2%
  - Evaluación: {nivel}
"""
    
    if 'TASA_AUSENTISMO' in df_filtrado.columns:
        ausentismo_actual = df_filtrado['TASA_AUSENTISMO'].mean()
        
        if ausentismo_actual < 2.8:
            nivel_aus = "EXCELENTE"
        elif ausentismo_actual < 3.5:
            nivel_aus = "BUENO"
        elif ausentismo_actual < 4.2:
            nivel_aus = "ATENCIÓN"
        else:
            nivel_aus = "CRÍTICO"
            
        comparative_analysis += f"""
ANÁLISIS COMPARATIVO AUSENTISMO:
  - Tasa actual Koaj: {ausentismo_actual:.2f}%
  - Benchmark sector: 2.8% - 4.0%
  - Evaluación: {nivel_aus}
"""
    
    return base_context + "\n\n" + comparative_analysis

# Versión mejorada de build_data_context (la original con algunos añadidos)
def build_data_context(df_filtrado: pd.DataFrame, mes: str, año: int, zona_seleccionada: str) -> str:
    ...
    try:
        ...
        promedios = df_filtrado[existing].mean().to_dict()
        
        resumen_num = "\n".join([f"  - {k}: {v:,.2f} (promedio: {promedios.get(k, 0):,.2f})" for k, v in totales.items()])
        ...
        if 'ZONA' in df_filtrado.columns:
            ...
            zona_prom = df_filtrado.groupby('ZONA')[existing].mean()
            por_zona = "\nResumen por zona (totales):\n" + zona_grp.to_string()
            por_zona += "\n\nPromedios por zona:\n" + zona_prom.to_string()
...
# Modificar la función ask_groq para que use el contexto enriquecido
def ask_groq_with_benchmark(messages_history: list, include_benchmark: bool = True) -> str:
    ...
    # Si se solicita, añadir instrucción específica para comparativas
    if include_benchmark and messages_history and messages_history[0]["role"] == "system":
        # Añadir instrucción al system prompt para que siempre compare
        benchmark_instruction = """
IMPORTANTE: En TODAS tus respuestas, cuando sea relevante:
1. Compara los datos de Koaj con los benchmarks del sector retail textil/fast-fashion
2. Menciona cómo se compara con competidores directos (Zara, H&M, Éxito)
3. Considera la estacionalidad en tus análisis
4. Ofrece recomendaciones basadas en las brechas identificadas
5. Usa un tono consultivo y orientado a acción
"""
...
# Al inicio, construir el contexto completo de mercado
MARKET_CONTEXT = build_market_benchmark_context() + "\n\n" + build_koaj_specific_context()

# Modificar la construcción del system prompt
...
current_ctx = build_data_context_enhanced(df_f, mes, año, zona_seleccionada)

if current_ctx != st.session_state.chat_data_context:
    st.session_state.chat_data_context = current_ctx
    if st.session_state.chat_messages:
        st.info("ℹ️ Contexto actualizado. Ahora puedes pedir comparativas con el mercado.")
...
# Input del usuario
user_input = st.chat_input("Pregunta algo sobre los datos... ej: ¿Cómo está nuestra rotación comparada con Zara?")
...
if user_input:
    ...
    system_prompt = f"""Eres un asesor estratégico de negocio especializado en retail textil y fast-fashion para KOAJ (Grupo Éxito).

CONTEXTO DE MERCADO (siempre úsalo para comparar):
{MARKET_CONTEXT}

DATOS ACTUALES DE KOAJ:
{st.session_state.chat_data_context}

INSTRUCCIONES ESPECÍFICAS:
1. Siempre que respondas, COMPARA los datos de Koaj con los benchmarks del sector
2. Menciona a competidores relevantes (Zara, H&M, Éxito, Falabella) cuando ayude al análisis
3. Considera la estacionalidad del mes actual ({mes})
4. Ofrece 2-3 recomendaciones accionables basadas en las brechas identificadas
5. Si los datos son insuficientes, sugiere qué información adicional sería útil
6. Sé conciso pero con suficiente detalle para toma de decisiones
7. Usa un tono profesional, consultivo y orientado a resultados

Ejemplo de formato de respuesta:
"La rotación actual de Koaj es X%, lo cual está [por debajo/encima/en línea] con el promedio del sector (Y%). Comparado con Zara (Z%) y H&M (W%), estamos [mejor/peor]. Para mejorar, recomendaría: 1) ... 2) ..."
"""

    api_messages = [{"role": "system", "content": system_prompt}]
    
    # Agregar historial (últimas 10 interacciones)
    for m in st.session_state.chat_messages[-10:]:
        api_messages.append({"role": m["role"], "content": m["content"]})

    with chat_container:
        with st.chat_message("assistant"):
            with st.spinner("🤔 Analizando datos y comparando con el mercado..."):
                response = ask_groq_with_benchmark(api_messages, include_benchmark=True)
...
if st.session_state.chat_messages:
    ...
    with col1:
        if st.button("🗑️ Limpiar", key="clear_chat"):
            ...
            st.rerun()
    with col2:
        st.caption("💡 Consejo: Pregunta cosas como '¿Cómo estamos comparados con Zara?' o '¿Nuestra rotación es buena para la industria?'")
