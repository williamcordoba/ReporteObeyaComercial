...
# ==========================
# FILTROS PRINCIPALES (ZONA + TIENDA)
# ==========================
...
tiendas_disponibles = sorted(df_zona['ALMACEN'].dropna().unique().tolist())

col_search, col_info = st.columns([3, 1])
with col_search:
    tienda_seleccionada = st.selectbox(
        "Seleccionar Tienda:",
        options=["TODAS"] + tiendas_disponibles,
        index=0,
        key="tienda_principal_select",
        help=f"Se muestran {len(tiendas_disponibles)} tiendas disponibles para la zona/período seleccionado."
    )
with col_info:
    st.metric("🏪 Tiendas disponibles", len(tiendas_disponibles))

if tienda_seleccionada != "TODAS":
    df_zona = df_zona[df_zona['ALMACEN'] == tienda_seleccionada].copy()
    st.info(f"🏪 Mostrando datos para: **{tienda_seleccionada}** ({len(df_zona)} registros)")

# ==========================
# APLICAR FILTROS ADICIONALES DEL SIDEBAR
# ==========================
df_f = df_zona.copy()

# Filtros del sidebar solo aplican si los filtros principales están en "TODAS"
if zona_sel != 'TODAS' and zona_seleccionada == "TODAS":
    df_f = df_f[df_f['ZONA'] == zona_sel]

if tienda_sel != 'TODAS' and tienda_seleccionada == "TODAS":
...
with tab2:
    ...
    with c1:
        ...
        fig.add_trace(go.Bar(
            ...
            marker=dict(color=top_rot['TASA_ROTACION'],
            ...
            hovertemplate=(
                '<b>%{y}</b><br>Tasa rotación: %{x:.1f}%<br>'
                'Activos: %{customdata[0]}<br>Retiros: %{customdata[1]}<br>'
                'Zona: %{customdata[2]}<br>Tasa mensual: %{customdata[3]:.1f}%<br>'
                'Tasa mes ant.: %{customdata[4]:.1f}%<extra></extra>'
            )
        ))
        fig.update_layout(title='🚪 Top 15 Tiendas – Mayor Tasa de Rotación',
        ...
        fig_cmp = go.Figure()
        fig_cmp.add_trace(go.Bar(name='Tasa mes anterior', x=top_rot['ALMACEN'],
                                  y=top_rot['TASA_ROTACION_ACT_MES_ANT'], marker_color=COLORS['accent']))
        fig_cmp.add_trace(go.Bar(name='Tasa mes actual', x=top_rot['ALMACEN'],
                                  y=top_rot['TASA_ROTACION_MENSUAL'], marker_color=COLORS['retiros']))
        fig_cmp.update_layout(title='Comparativa Tasa Rotación: Mes Anterior vs Actual (Top 15)',
...
with tab3:
    ...
    with c1:
        ...
        fig.add_trace(go.Bar(
            ...
            texttemplate='%{text}%', textposition='outside',
            marker=dict(color=top_aus['TASA_AUSENTISMO'],
            ...
            hovertemplate=(
                ...
                'Activos: %{customdata[0]}<br>Total ausentes: %{customdata[1]}<br>'
                'Zona: %{customdata[2]}<br>🏥 Médico: %{customdata[3]}<br>'
                '⚖️ Legal: %{customdata[4]}<br>🗂️ Admin: %{customdata[5]}<br>'
                '📅 Días: %{customdata[6]}<br>⏱️ Horas: %{customdata[7]:.0f}<extra></extra>'
            )
        ))
        fig.update_layout(title='😷 Top 15 Tiendas – Mayor Tasa de Ausentismo',
        ...
        fig2.update_layout(title='Composición del Ausentismo por Zona',
...
with tab4:
    ...
    with c1:
        ...
        fig.add_trace(go.Bar(
            ...
            texttemplate='%{text}%', textposition='outside',
            marker=dict(color=top_acc['TASA_ACCIDENTALIDAD'],
                        colorscale=[[0, '#c8f7c5'], [1, COLORS['accidentes']]]),
            customdata=top_acc[['TOTAL_ACTIVOS','TOTAL_ACCIDENTES','ZONA']],
            hovertemplate=(
                '<b>%{y}</b><br>Accidentalidad: %{x:.1f}%<br>'
                'Activos: %{customdata[0]}<br>Accidentes: %{customdata[1]}<br>'
                'Zona: %{customdata[2]}<extra></extra>'
            )
        ))
        fig.update_layout(title='🚑 Top 15 Tiendas – Mayor Tasa de Accidentalidad',
                          xaxis_title='Tasa (%)', yaxis_title='',
                          plot_bgcolor='white', paper_bgcolor='white',
                          height=500, margin=dict(l=200))
        st.plotly_chart(fig, use_container_width=True)
...
with tab5:
    ...
    fig_t.update_layout(title='Evolución de Tasas por Período',
    ...
    fig_act.update_layout(title='Personal Activo por Mes y Año',
...
if tienda_seleccionada != "TODAS":
    df_oficio = df_oficio[df_oficio['ALMACEN'] == tienda_seleccionada]
...
if not df_oficio.empty and 'NOM_OFICIO' in df_oficio.columns:
    ...
    with c1:
        ...
        fig_cargo.add_trace(go.Bar(
            y=top_cargo['NOM_OFICIO'], x=top_cargo['TOTAL_ACTIVOS'],
            orientation='h', text=top_cargo['TOTAL_ACTIVOS'],
            texttemplate='%{text:,}', textposition='outside',
            marker=dict(color=COLORS['primary']),
            hovertemplate='<b>%{y}</b><br>Activos: %{x:,}<extra></extra>'
        ))
        fig_cargo.update_layout(title='🏆 Top 15 Cargos por Cantidad de Activos',
                                 xaxis_title='Total Activos', yaxis_title='',
                                 plot_bgcolor='white', paper_bgcolor='white',
    ...
    with c2:
        ...
        fig_treemap = px.treemap(top_20, path=['NOM_OFICIO'], values=variable_sel,
                                  title=titulo, color=variable_sel,
    ...
    st.dataframe(
        cargo_table.style.format({
            'TOTAL_ACTIVOS': '{:,.0f}', 'RETIROS': '{:,.0f}',
            'TOTAL_AUSENTISMO': '{:,.0f}', 'TOTAL_ACCIDENTES': '{:,.0f}',
            'TASA_ROTACION': '{:.1f}%', 'TASA_AUSENTISMO': '{:.1f}%',
...
zonas_tasas = df_f.groupby('ZONA').agg({
    'TOTAL_ACTIVOS': 'sum', 'RETIROS': 'sum',
    'TOTAL_AUSENTISMO': 'sum', 'TOTAL_ACCIDENTES': 'sum'
}).reset_index()
...
fig_zonas = go.Figure()
fig_zonas.add_trace(go.Bar(name='Rotación %', x=zonas_tasas['ZONA'], y=zonas_tasas['TASA_ROTACION'],
                            marker_color=COLORS['retiros'], text=zonas_tasas['TASA_ROTACION'].round(1),
                            texttemplate='%{text}%', textposition='outside'))
fig_zonas.add_trace(go.Bar(name='Ausentismo %', x=zonas_tasas['ZONA'], y=zonas_tasas['TASA_AUSENTISMO'],
                            marker_color=COLORS['ausentismo'], text=zonas_tasas['TASA_AUSENTISMO'].round(1),
                            texttemplate='%{text}%', textposition='outside'))
fig_zonas.add_trace(go.Bar(name='Accidentalidad %', x=zonas_tasas['ZONA'], y=zonas_tasas['TASA_ACCIDENTALIDAD'],
                            marker_color=COLORS['accidentes'], text=zonas_tasas['TASA_ACCIDENTALIDAD'].round(1),
                            texttemplate='%{text}%', textposition='outside'))
fig_zonas.update_layout(title='📈 Comparación de Tasas por Zona', barmode='group',
                         xaxis_title='Zona', yaxis_title='Tasa (%)',
...
if not df_retiros.empty and 'RANGO_PERMANENCIA' in df_retiros.columns:
    ...
    fig_rango.add_trace(go.Bar(
        y=rango_agg['Rango de Permanencia'], x=rango_agg['Total Retiros'],
        orientation='h', text=rango_agg['Total Retiros'],
        texttemplate='%{text:,}', textposition='outside',
        marker=dict(color=rango_agg['Total Retiros'],
                    colorscale=[[0, COLORS['accent']], [1, COLORS['danger']]]),
        hovertemplate='<b>%{y}</b><br>Retiros: %{x:,}<extra></extra>'
    ))
    fig_rango.update_layout(title='📊 Retiros por Rango de Permanencia',
                             xaxis_title='Total Retiros', yaxis_title='Rango de Permanencia',
                             plot_bgcolor='white', paper_bgcolor='white',
    ...
    rango_cargo = df_retiros.groupby(['NOM_OFICIO', 'RANGO_PERMANENCIA']).agg({'RETIROS': 'sum'}).reset_index()
    ...
    if not rango_cargo.empty:
        ...
        for i, rango in enumerate(cols_orden):
            fig_rango_cargo.add_trace(go.Bar(
                name=rango, y=rango_pivot.index, x=rango_pivot[rango],
                orientation='h', marker_color=colores_rango[i],
        ...
        fig_rango_cargo.update_layout(
            ...
            barmode='stack', xaxis_title='Total Retiros', yaxis_title='Cargo',
            plot_bgcolor='white', paper_bgcolor='white',
            height=500, margin=dict(l=250),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
        )
        st.plotly_chart(fig_rango_cargo, use_container_width=True)

        st.markdown("#### 📊 Resumen por Cargo y Permanencia")
        rango_cargo_display = rango_cargo_top.sort_values(['NOM_OFICIO', 'RETIROS'], ascending=[True, False])
        st.dataframe(rango_cargo_display.style.format({'RETIROS': '{:,.0f}'}),
...
<div style="text-align:center; padding:20px;
    ...
    <p style="margin:5px 0;">
        ...
        <b>Tienda:</b> {tienda_seleccionada} |
