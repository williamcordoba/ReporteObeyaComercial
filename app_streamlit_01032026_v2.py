...
# ==========================
st.set_page_config(
    page_title="Obeya Comercial 2026",
    page_icon="📊",
    layout="wide",
...
if metas_disponibles:
    ...
    vista_meta = st.radio(
        "Ver metas por:",
        ["📊 Consolidado", "🏪 Por Tienda", "🌍 Por Isócrona / Zona"],
        horizontal=True,
        key="vista_meta"
    )
...
with tab1:
    ...
    with c2:
        tiendas_gen = df_f.nlargest(15, 'TOTAL_ACTIVOS')[['ALMACEN','TOTAL_ACTIVOS','RETIROS','TOTAL_AUSENTISMO','TOTAL_ACCIDENTES']].copy()
        fig2 = go.Figure()
        for trace_name, col, color in [
            ('👥 Activos',    'TOTAL_ACTIVOS',   COLORS['primary']),
            ('🔄 Rotación',   'RETIROS',          COLORS['retiros']),
...
with tab2:
    ...
    with c1:
        top_rot = df_f.nlargest(15, 'RETIROS')[
        ...
        fig.update_layout(
            title='🔄 Top 15 Tiendas – Mayor Rotación',
        ...
        if 'TASA_ROTACION_ACT_MES_ANT' in top_rot.columns and 'TASA_ROTACION_MENSUAL' in top_rot.columns:
            ...
            fig_cmp.update_layout(
                title='Comparativa Rotación: Mes Anterior vs Actual (Top 15)',
...
with tab3:
    ...
    with c1:
        top_aus = df_f.nlargest(15, 'TOTAL_AUSENTISMO')[
        ...
        fig.update_layout(
            title='😷 Top 15 Tiendas – Mayor Ausentismo',
...
with tab4:
    ...
    with c1:
        top_acc = df_f.nlargest(15, 'TOTAL_ACCIDENTES')[
        ...
        fig.update_layout(
            title='🚑 Top 15 Tiendas – Mayor Accidentalidad',
...
# ==========================
# TOP CARGOS CON MÁS ROTACIÓN
# ==========================
st.markdown("### 👔 Top Cargos con Más Rotación")
...
if not df_oficio.empty and 'NOM_OFICIO' in df_oficio.columns:
    ...
    cargo_rot = cargo_rot[cargo_rot['RETIROS'] > 0].sort_values('RETIROS', ascending=False).head(20)
    ...
    fig_top_cargo.update_layout(
        title='Top Cargos por Cantidad de Rotaciones',
        xaxis_title='Total Rotaciones',
        yaxis_title='',
        plot_bgcolor='white', paper_bgcolor='white',
        height=500, margin=dict(l=280)
    )
    st.plotly_chart(fig_top_cargo, use_container_width=True)
else:
    st.info("ℹ️ No hay datos de cargo disponibles para los filtros seleccionados.")
...
# MOTIVOS DE RETIRO
# ==========================
st.markdown("### 📋 Motivos de Rotación")

if 'MOTIVO' in df_oficio.columns:
    ...
    if not df_motivos.empty:
        ...
        c_mot1, c_mot2 = st.columns([3, 2])

        with c_mot1:
            fig_mot = go.Figure()
            fig_mot.add_trace(go.Bar(
                y=motivo_agg['Motivo'],
                x=motivo_agg['Total Rotaciones'],
                orientation='h',
                text=motivo_agg['Total Rotaciones'].astype(int),
                texttemplate='%{text:,}',
                textposition='outside',
                marker_color=COLORS['retiros'],
            ...
            fig_mot.update_layout(
                title='Rotaciones por Motivo',
                xaxis_title='Total Rotaciones', yaxis_title='',
                plot_bgcolor='white', paper_bgcolor='white',
                height=max(350, len(motivo_agg) * 45),
                margin=dict(l=320)
            )
        ...
        st.info("ℹ️ No hay registros de motivos de retiro para los filtros seleccionados.")
else:
    st.info("ℹ️ La columna MOTIVO no se encontró en los datos.")
...
if not df_retiros.empty and 'RANGO_PERMANENCIA' in df_retiros.columns:
    ...
    if not rango_cargo.empty:
        top_cargos_retiro = rango_cargo.groupby('NOM_OFICIO')['RETIROS'].sum().nlargest(15).index
        ...
        # Ordenar por total de retiros descendente
        ...
        fig_rango_cargo.update_layout(
            title='Top Cargos – Rotación por Rango de Permanencia (barras apiladas)',
            barmode='stack',
            xaxis_title='Total Rotaciones',
            yaxis_title='Cargo',
            plot_bgcolor='white', paper_bgcolor='white',
...
with tc3:
    sort_opts = [c for c in ['TOTAL_ACTIVOS','RETIROS','TOTAL_AUSENTISMO',
                               'TOTAL_ACCIDENTES','ALMACEN'] if c in df_f.columns]
